...
public class PublicMetadataHpackBenchmark {
  ...
  private static final Set<AsciiString> HIGH_CARDINALITY_NAMES =
      Collections.unmodifiableSet(new HashSet<>(Arrays.asList(
          AsciiString.cached("x-request-id"),
          AsciiString.cached("x-cloud-trace-id"),
          AsciiString.cached("x-request-context"),
          AsciiString.cached("x-correlation-id"),
          AsciiString.cached("baggage"))));

  public enum HeaderPolicy {
    INDEX_HIGH_CARDINALITY,
    NEVER_INDEX_HIGH_CARDINALITY
  }
  ...
  @Param({"INDEX_HIGH_CARDINALITY", "NEVER_INDEX_HIGH_CARDINALITY"})
  ...
  public void setUpTrial() throws Http2Exception {
    ...
    Http2HeadersEncoder.SensitivityDetector detector = Http2HeadersEncoder.NEVER_SENSITIVE;
    if (headerPolicy == HeaderPolicy.NEVER_INDEX_HIGH_CARDINALITY) {
      detector = new Http2HeadersEncoder.SensitivityDetector() {
        @Override
        public boolean isSensitive(CharSequence name, CharSequence value) {
          return HIGH_CARDINALITY_NAMES.contains(AsciiString.of(name));
        }
      };
    }
    encoder = new DefaultHttp2HeadersEncoder(detector);
    encoder.maxHeaderTableSize(4_096);
